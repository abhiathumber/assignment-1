...
variable "loadbalancer-pubip" {
    default = "2235-loadbalancer-pubip"
}

variable "loadbalancer" {
    type = map(string) 
    default = {
        name    = "2235-loadbalancer"
        frontend_ip_configuration = "pubip-2235"
    }
}

variable "backend_address_pool" {
    default = "2235-backendaddresspool"
}

variable "loadbalancer_pool_association" {
    default = "2235-loadbalancer-pool-association"
}

variable "loadbalancer_rule" {
    type = map(string) 
    default = {
        name = "2235-loadbalancer-rule"
        protocol = "Tcp"
        frontend_port = 3389
        backend_port = 3389
